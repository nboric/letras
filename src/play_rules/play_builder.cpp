...
PlayBuilder::PlayBuilder()
{
    ...
    rules_.push_back(std::make_unique<DictCheck>());
}

bool PlayBuilder::build(Play& play, const Board& board, const Dict& dict, std::string& rule_name,
    std::string& reason) const
{
    for (const auto& rule : rules_)
    {
        if (!rule->isValid(play, board, dict, reason))
        {
